...
public class TestController {
	...
	public CMRespDto<String> findByUsername(String username){ //for each 사용.
	...
	public CMRespDto<Integer> findByUsercode(String username){ //일반 for문 사용.
		for(int i = 0; i < usernames.length; i++) {
			if(usernames[i].equals(username)) {
				return new CMRespDto<Integer>(1, username + "의 usercode를 찾음", usercode[i]);
			}
		}
		return new CMRespDto<Integer>(-1, username + "의 usercode를 찾지 못 함", 0);
	}
}
